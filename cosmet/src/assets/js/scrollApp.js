class scrollApp {
	...
	checkScroll() {
		...
		for (var i = 0; i < this.scrolls.length; i++) {
			if ( this.scrolls[i]['hide'] ) {
				if ( this.scrolls[i]['scroll']  < ( document.querySelector('body').scrollTop + document.querySelector('body').clientHeight)   ) {
	...
	animationClipover(elem) {
		...
		// overlay add work
		elem.querySelector('.overlay').style.animation = "clipover2 " + animationDuration + " linear";
		setTimeout(function(){
			elem.classList.add('clipover--active');
		}, ( elem.getAttribute('data-animation-duration') / 2 ) );
	}
	...
	animationTyping(elem) {
		...
		for (let char of animationText) {
			...
			if ( animationLetter == j ) {
				setTimeout(function(){
					let curText = elem.innerHTML;
					let newText =  curText + "<em><strong></strong>"+char+"</em>";
